...
public class ProjectsService {
    ...
    public ProjectOutputDTO createProject(ProjectInputDTO dto, HttpServletRequest request, HttpServletResponse response) {
        ProjectOutputDTO res = new ProjectOutputDTO()
        .setStatus(Status.SUCCESS);
        AuthenticationOutputDTO userRes = userClient.getUserDetails(SecurityContextHolder.getContext().getAuthentication().getPrincipal().toString());
        ...
        dto.getProject()
        .setOwnerId(userRes.getUser().getId())
    ...
    public ProjectOutputDTO editProject(ProjectInputDTO dto, HttpServletRequest request, HttpServletResponse response) {
        ProjectOutputDTO res = new ProjectOutputDTO()
        .setStatus(Status.SUCCESS);
        AuthenticationOutputDTO userRes = userClient.getUserDetails(SecurityContextHolder.getContext().getAuthentication().getPrincipal().toString());
        if(userRes.getStatus() == Status.ERROR) {
        ...
        if(actualProj.getEnterpriseOwner().equals(true)) {
            return res.setStatus(Status.ERROR).setMessage("Enterprise Project");
        }
    ...
    public ProjectOutputDTO forkProject(ProjectInputDTO dto, HttpServletRequest request, HttpServletResponse response) {
        ProjectOutputDTO res = new ProjectOutputDTO()
        .setStatus(Status.SUCCESS);
        ...
        if(newProj.getEnterpriseOwner().equals(true)) {
            return res.setStatus(Status.ERROR).setMessage("Enterprise Project");
        }
        if(!newProj.getAllowForks() || (!newProj.getVisiblePublic() && !newProj.getContributors().contains(userRes.getUser().getId()) && !newProj.getViewers().contains(userRes.getUser().getId()))) {
    ...
    public ProjectOutputDTO deleteProject(ProjectInputDTO dto, HttpServletRequest request, HttpServletResponse response) {
        ProjectOutputDTO res = new ProjectOutputDTO()
        .setStatus(Status.SUCCESS);
        ...
        if(actualProject.getEnterpriseOwner().equals(true)) {
            return res.setStatus(Status.ERROR).setMessage("Enterprise Project");
        }
        if(userRes.getUser().getId() != actualProject.getOwnerId()) {
            return res.setStatus(Status.ERROR).setMessage("Invalid ownership credentials");
        }
    ...
    public ProjectOutputDTO retrieveProject(ProjectInputDTO dto, HttpServletRequest request, HttpServletResponse response) {
        ProjectOutputDTO res = new ProjectOutputDTO()
        .setStatus(Status.SUCCESS);
        ...
        if(!actualProject.getVisiblePublic() && actualProject.getOwnerId() != userRes.getUser().getId() && 
    ...
    public ProjectOutputDTO searchProjects(ProjectInputDTO dto, HttpServletRequest request, HttpServletResponse response) {
        ProjectOutputDTO res = new ProjectOutputDTO()
        .setStatus(Status.SUCCESS);
        AuthenticationOutputDTO userRes = userClient.getUserDetails(SecurityContextHolder.getContext().getAuthentication().getPrincipal().toString());
        if(userRes.getStatus() == Status.ERROR) {
        ...
        .filter(result -> (result.getVisiblePublic() || result.getContributors().contains(userRes.getUser().getId()) || result.getViewers().contains(userRes.getUser().getId())))
        .map(elem -> elem.clearSensistiveInfo())
        .collect(Collectors.toList());
        return res.setProjects(results);
    }

    public ProjectOutputDTO starProject(ProjectInputDTO dto, HttpServletRequest request, HttpServletResponse responose) {
        ProjectOutputDTO res = new ProjectOutputDTO()
        .setStatus(Status.SUCCESS);
        AuthenticationOutputDTO userRes = userClient.getUserDetails(SecurityContextHolder.getContext().getAuthentication().getPrincipal().toString());
        if(userRes.getStatus() == Status.ERROR) {
    ...
    public ProjectOutputDTO getForks(ProjectInputDTO dto, HttpServletRequest request, HttpServletResponse response) {
        ProjectOutputDTO res = new ProjectOutputDTO()
        .setStatus(Status.SUCCESS);
        AuthenticationOutputDTO userRes = userClient.getUserDetails(SecurityContextHolder.getContext().getAuthentication().getPrincipal().toString());
        if(userRes.getStatus() == Status.ERROR) {
    ...
    public ProjectOutputDTO getTags(ProjectInputDTO dto, HttpServletRequest request, HttpServletResponse response) {
        ProjectOutputDTO res = new ProjectOutputDTO()
        .setStatus(Status.SUCCESS);
        AuthenticationOutputDTO userRes = userClient.getUserDetails(SecurityContextHolder.getContext().getAuthentication().getPrincipal().toString());
        if(userRes.getStatus() == Status.ERROR) {
